...
use crate::bitslice::{
    bit_add_wrapping, bit_splat, comparator, extend, ragged_array_popcount, BitSlice,
    BlockTranspose,
};
use rayon::prelude::*;
...
fn unit_counts_full<T: BitSlice + Copy, const I: usize, const N: usize, const P: usize>(
    partial_sum: &[T; P],
    inputs: &[T; I],
    target_bit: &T,
    thresholds: &[[T; P]; N],
...
fn compute_base_sum<T: BitSlice + Copy, const L: usize, const P: usize>(
    ...
    //let set_bits: Vec<T> = table.iter().map(|&(i, s)| bits[i].xor(T::splat(s))).collect();
    //ragged_array_popcount(&set_bits)
    //[T::zeros(); P]
}
                    ...
                    |mut acc, chunk| {
                        for o in 0..(64 * O) {
                            chunk.iter().for_each(|(input, target)| {
                                let base_sum = compute_base_sum(&table[o].0, input);
                                let exp_bits =
                                    extract_exp_bits::<T, { 64 * I }, E>(&table[o].1, input);
                                exp_count::<T, N, P, E>(
                                    &base_sum,
                                    &exp_bits,
                                    &target[o],
