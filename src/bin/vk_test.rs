...
mod fast_obj_update {
    vulkano_shaders::shader! {
        ty: "compute",
        path: "shaders/fast_mirror_update_test.glsl",
    }
}
    ...
    fn is_good(&self, head_words: &[u32; 10]) -> bool {
        ...
        for c in 0..10 {
            is_good = ((c == self.true_class as usize)
                | ((self.embedding_word ^ head_words[c]).count_ones() < max_obj))
                && is_good;
        }
        is_good
    }
            ...
            > for FastExampleMirrored
        {
            fn new(
                ...
                let input_words = if input_x == 0 {
                    [
                        input[0][input_y][input_pixel_word],
                        input[2][input_y][input_pixel_word],
                    ]
                } else if input_x == 2 {
                    [
                        input[2][input_y][input_pixel_word],
                    ...
                    [
                        input[1][input_y][input_pixel_word],
                        input[1][input_y][input_pixel_word],
                    ]
                };
                FastExampleMirrored {
                    ...
                    input_partial_sums: [
                        weights[embedding_word][embedding_bit].normal_hamming_distance(&input)
                            - weights[embedding_word][embedding_bit][input_x][input_y]
                                [input_pixel_word]
        ...
        vulkano::descriptor::descriptor_set::PersistentDescriptorSet<
            std::sync::Arc<
                vulkano::pipeline::ComputePipeline<
                    vulkano::descriptor::pipeline_layout::PipelineLayout<fast_obj_update::Layout>,
                >,
            >,
            (
                (
...
impl<
        InputPatch: Sync + GetWord + BitLen,
        Weights: Sync + GetPatch<InputPatch>,
        Embedding: Sync,
    > FastCacheVKObjEval<InputPatch, Weights, Embedding>
where
    ...
    [Embedding; 10]: GetHeadWords,
{
    fn new_vk_fast_cache(
        ...
        let data_vec: Vec<_> = examples
            ...
            .collect();
        let (cache_buffer, _) =
            ImmutableBuffer::from_iter(data_vec.iter().cloned(), BufferUsage::all(), queue.clone())
                .unwrap();
        ...
        FastCacheVKObjEval {
            ...
            weights_patch_word: weights
                .get_patch(embedding_index % 16)
                .get_word(embedding_index / 16),
        }
    }

    fn sum_obj(&self) -> Vec<u32> {
        ...
        .unwrap()
        .dispatch(
            [(self.n_examples as f64 / 2f64).ceil() as u32, 1, 1],
        ...
        data_buffer_content.iter().cloned().collect()
    }
}

//const N_EXAMPLES: usize = 50000 * 900 / 4;
//const N_EXAMPLES: usize = 104729 * 13 * 7;
const N_EXAMPLES: usize = 256;
const BATCH_SIZE: usize = 1;
const EMBEDDING_BIT_INDEX: usize = 0;
...
fn main() {
    ...
    let fast_cache = FastCacheVKObjEval::new_vk_fast_cache(
        &examples,
        &full_weights,
        &full_head,
        EMBEDDING_BIT_INDEX,
    ...
    println!("{:?}", cf_gpu_sum.len());

    let vk_like_sum: u64 = examples
        .iter()
        .zip(cf_gpu_sum.iter())
        .map(|((class, input), gpu_result)| {
            let true_embedding = full_weights.apply(input);
            let true_correct = full_head.is_correct(*class as u8, true_embedding);
            dbg!(*gpu_result);
            assert_eq!(*gpu_result, true_correct as u32);
            true_correct as u64
        })
        .sum();

    //dbg!(vk_like_sum);
    //assert_eq!(vk_like_sum, fc_gpu_sum);
}
