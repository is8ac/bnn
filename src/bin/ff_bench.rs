    ...
    i8: Element<C>,
{
    ...
    fn egd_descend(
        self,
        class_acts: &Vec<<i32 as Element<C>>::Array>,
...
fn main() {
    ...
    let aux_weights = (0..10).fold(aux_weights, |weights, i| {
        weights
            .egd_descend(&class_act_cache, &patch_bags, &patch_acts, 1)
            .egd_descend(&class_act_cache, &patch_bags, &patch_acts, -1)
    });

    let (mut cur_sum_loss, n_correct, n) =
