...
fn main() {
    ...
    let base_path = Path::new("params/vk_noskip_wide_2");
    ...
    let mut l0_images: Vec<(usize, [[[u32; 2]; 32]; 32])> =
        <[[[[[u32; 3]; 3]; 3]; 16]; 2]>::train_from_images(
            &mut rng,
            &eval_creator,
            &chan3_images,
            &base_path.join("b1_l0_c3_3-2"),
            11,
            HEAD_UPDATE_FREQ,
            &log_file_path,
            2,
    ...
    for l in 1..20 {
        l0_images = <[[[[[u32; 2]; 3]; 3]; 16]; 2]>::train_from_images(
            &mut rng,
            &eval_creator,
            &l0_images,
            &base_path.join(format!("b1_l{}_c3_2-2", l)),
            11,
            HEAD_UPDATE_FREQ,
            &log_file_path,
            1,
    ...
    let mut l0_images: Vec<(usize, [[[u32; 4]; 16]; 16])> =
        <[[[[[u32; 2]; 2]; 2]; 16]; 4]>::train_from_images(
            &mut rng,
            &eval_creator,
            &l0_images,
