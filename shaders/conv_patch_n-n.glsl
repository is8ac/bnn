...
layout(set = 0, binding = 0) buffer Prms {
  uint[${OUTPUT_LEN}][32][${PATCH_SIZE}][${PATCH_SIZE}][${INPUT_LEN}] data;
}
prms;
...
layout(set = 0, binding = 2) buffer InputPatch {
  uint[${PATCH_SIZE}][${PATCH_SIZE}][${INPUT_LEN}] data[];
}
input_patch;
...
void main() {
  ...
  const uint threshold = (32 * ${INPUT_LEN} * (${PATCH_SIZE} * ${PATCH_SIZE})) / 2;
  uint x;
  uint y;
  uint b;
  uint o;
  ...
  if (pc.full_apply == 1) {
    for (e = 0; e < ${OUTPUT_LEN}; e += 1) {
      ...
      for (b = 0; b < 32; b += 1) {
        sum_bits = 0;
        for (x = 0; x < ${PATCH_SIZE}; x += 1) {
          for (y = 0; y < ${PATCH_SIZE}; y += 1) {
