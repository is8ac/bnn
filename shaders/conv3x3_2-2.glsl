...
layout(set = 0, binding = 0) buffer Prms {
  uint[2][32][3][3][2] data;
}
prms;

layout(set = 0, binding = 1) buffer ObjHead { uint[10][2] data; }
head;
...
layout(set = 0, binding = 4) buffer Embedding { uint[2] data[]; }
embedding;
...
void main() {
  ...
  const uint threshold = (32 * 2 * (3 * 3)) / 2;
  uint x;
  uint y;
  uint b;
  uint o;
